...
describes what users get. Pick a format with `-o`, or let idigbio choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
idigbio <command> -o table   # aligned columns for reading
idigbio <command> -o jsonl   # one JSON object per line, for piping
idigbio <command> -o json    # a single JSON array
idigbio <command> -o csv     # spreadsheet friendly
idigbio <command> -o tsv     # tab-separated
idigbio <command> -o url     # just the URL column
idigbio <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
idigbio <command> --fields id,title,url
```
...
```bash
idigbio <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
idigbio <command>            # a table, because this is a terminal
idigbio <command> | wc -l    # JSONL, because this is a pipe
```
